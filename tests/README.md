...
`update_happy.py`
- This is not a test file, but I will provide an explanation for the file's functionality. This file updates the `happy.json` prompt engineering file whenever changes are made to Happy's prompt engineering. `happy.json` is a file that all files in `tests` use to implement Happy. Whenever changes are made to Happy's prompt engineering, this design, in which only this singular file is updated and ran to store changes in `happy.json`, is more efficient than going through every test file and changing the prompt training each time to reflect parameters in the main `src/Hap.py`.